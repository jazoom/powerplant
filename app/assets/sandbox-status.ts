...
export function initSandboxStatus(root: HTMLElement): IslandInstance {
    ...
    const submit = () => {
        if (root.dataset.sandboxActive !== "true") {
            return;
        }
        form()?.requestSubmit();
    };
    ...
    return {
        reconcile(context) {
            ...
            if (context.detail.outcome === "applied-patch") {
                if (!context.detail.targetIds.includes("sandbox-status")) {
                    return;
                }
                clearTimer();
                submit();
